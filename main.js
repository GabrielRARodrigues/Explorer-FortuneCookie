...
const titleElement = getElementByDataAtribute('title')

/* Fortune Cookie Elements */
const fortuneCookieMessageElement = getElementByDataAtribute('cookie-message')
const fortuneCookieImageContainerElement =
  getElementByDataAtribute('cookie-image')
const fortuneCookieImageElement =
  fortuneCookieImageContainerElement.querySelector('img')
const fortuneCookieButtonElement = getElementByDataAtribute('cookie-button')

// Comum Variables

/* Fortune Cookie Variables*/
let fortuneCookieIsOpen = false
...
/* Default Variables */
const defaultTitleContent = 'Qual é a sua sorte de hoje?'
const defaultCookieImageAlt = 'Figura de um biscoito da sorte fechado'
const defaultCookieImageSrc = './assets/images/closed-cookie.svg'

// General Functions

function getElementByDataAtribute(atribute) {
  const element = document.querySelector(`[data-${atribute}]`)

  return element
}

function toggleAtribute(element, atribute) {
  fortuneCookieIsOpen = !fortuneCookieIsOpen

  if (element.getAttributeNames().includes(atribute)) {
...
function drawFortuneCookiePhrase() {
  const randomIndex = Math.floor(Math.random() * fortuneCookiePhrases.length)
  console.log(randomIndex < fortuneCookiePhrases.length)

  const selectedPhrase = fortuneCookiePhrases[randomIndex]
...
function openFortuneCookie() {
  if (fortuneCookieIsOpen) {
    return
  }

  toggleAtribute(appElement, 'data-cookie-is-open')
  fortuneCookieMessageElement.textContent = drawFortuneCookiePhrase()
  fortuneCookieImageElement.src = defaultCookieImageSrc.replace(
    'closed',
    'opened'
  )
  fortuneCookieImageElement.alt = defaultCookieImageAlt.replace(
    'fechado',
    'aberto'
  )
  titleElement.textContent = 'Aqui está a sua sorte de hoje:'
}

function reset() {
  toggleAtribute(appElement, 'data-cookie-is-open')
  fortuneCookieImageElement.src = defaultCookieImageSrc
  fortuneCookieImageElement.alt = defaultCookieImageAlt
  titleElement.textContent = defaultTitleContent
}

// Events

fortuneCookieImageContainerElement.addEventListener('click', openFortuneCookie)

fortuneCookieButtonElement.addEventListener('click', reset)
